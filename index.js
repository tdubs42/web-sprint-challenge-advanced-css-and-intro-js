...
function getArtistByIndex(anArray, n) {
	return `the artist at index ${artists[n].id} is ${artists[n].name}`;
}
...
function get20s(anArray) {
	...
	for (let i = 0; i < anArray.length; i++) {
		birth = anArray[i].years.slice(0, 3);
		parseInt(birth, 10);

		if (birth >= 1900 && birth <= 2000) {
			death = anArray[i].years.slice(7, 10);
			parseInt(death, 10);
...
function removeArtist(anArray, n) {
	artists.splice(n, 1);
...
function addArtist(anArray) {
	const tdubs = {
		...
		nationality: 'Mixed',
		bio:
			'I strive to solve the worlds problems before people realize they have a problem.',
	};
